...
class Optimizer(object):
    def __init__(self, embeddings, latent_inters, latent_varies,
                 degrees, edge_type2dim, placeholders,
                 margin=0.1, neg_sample_weights=1., batch_size=100):
        self.batch_edge_type_idx = None
        self.embeddings= embeddings
        self.latent_inters = latent_inters[0]
        self.latent_varies = latent_varies[0]
        # self.edge_types = edge_types
        self.degrees = degrees
        self.edge_type2dim = edge_type2dim
        self.obj_type2n = self.edge_type2dim[0]
        self.margin = margin
        ...
        print("10005656656651566")
        print( self.row_inputs)
        # obj_type_n = [self.obj_type2n[i] for i in range(len(self.embeddings))]
        obj_type_n = self.obj_type2n
        self.obj_type_lookup_start = 0
        self.obj_type_lookup_end = obj_type_n
        ...
        self.neg_samples = tf.gather(neg_samples_list, 0)
        print("dddddd")
        print(neg_samples)
    ...
    def batch_predict(self, row_inputs, col_inputs):
        ...
        ind_start = self.obj_type_lookup_start
        ind_end = self.obj_type_lookup_end
        ...
        latent_inter = self.latent_inters
        latent_var = self.latent_varies
